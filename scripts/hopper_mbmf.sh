...
then
python plot_mbmf.py --trpo_dir=$trpo_dir --std_on_mlp_policy=$std_on_mlp_policy --which_agent=$which_agent --run_nums 21 22 23 --seeds ${seeds[0]} ${seeds[1]} ${seeds[2]}
fi

if [ $how_many_seeds -eq 2 ]
then
python plot_mbmf.py --trpo_dir=$trpo_dir --std_on_mlp_policy=$std_on_mlp_policy --which_agent=$which_agent --run_nums 21 22 --seeds ${seeds[0]} ${seeds[1]}
fi

if [ $how_many_seeds -eq 1 ]
then
python plot_mbmf.py --trpo_dir=$trpo_dir --std_on_mlp_policy=$std_on_mlp_policy --which_agent=$which_agent --run_nums 21 --seeds ${seeds[0]}
fi
